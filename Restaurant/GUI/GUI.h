...
private:
	...
	Order * OrdListTemp[MaxPossibleOrdCnt];
...
public:
	...
	// Output Functions  ---------------------------
	void PrintMessage(string msg, int x = 10, int y = 5) const; // prints a message in the status bar
	void ClearStatusBar() const;    // clears the status bar


	void UpdateInterface();
	void AddOrderForDrawing(Order*);	//Adds a new order to the drawing list
	void ResetDrawingList();		//resets drawing list (should be called every timestep after drawing)
	void Removedrawing(Order*);
